...
@Component({
  ...
  // TEMPLATE is adjusted to use the correct properties from service interfaces
  template: `
            ...
            <tbody>
              <tr *ngFor="let doctor of paginatedDoctors">
                ...
                <td>
                  ...
                  <button class="btn btn-sm btn-outline-danger" (click)="confirmDeleteDoctor(doctor)" title="Supprimer"><i class="fas fa-trash"></i></button>
                </td>
              </tr>
            ...
            <tbody>
              <tr *ngFor="let patient of paginatedPatients">
                ...
                <td>
                  ...
                  <button class="btn btn-sm btn-outline-danger" (click)="confirmDeletePatient(patient)" title="Supprimer"><i class="fas fa-trash"></i></button>
                </td>
              </tr>
...
export class AdminDashboardComponent implements OnInit {
  ...
  private loadDoctors(): void {
    ...
    this.doctorService.getAllDoctors().pipe(
      ...
      error: (err) => { this.error = "Erreur chargement médecins."; }
    });
  }
  ...
  private loadPatients(): void {
    ...
    this.patientService.getAllPatients().pipe(
      ...
      error: (err) => { this.error = "Erreur chargement patients."; }
    });
  }
  ...
  private loadConsultations(): void {
    ...
    this.consultationService.getConsultations().pipe(
      ...
      error: (err) => { this.error = "Erreur chargement consultations."; }
    });
  }
  ...
  private deleteDoctor(userId: string): void {
    this.doctorService.deleteDoctor(userId).subscribe({
      ...
      error: (err) => { this.error = 'La suppression a échoué.'; }
    });
  }
  ...
  addDoctor() { this.router.navigate(['/admin/doctors/add']); }
  editDoctor(doctor: Doctor) { this.router.navigate(['/admin/doctors/edit', doctor.user_id]); }
  viewDoctor(doctor: Doctor) { this.router.navigate(['/admin/doctors/view', doctor.user_id]); }

  addPatient() { this.router.navigate(['/admin/patients/add']); }
  editPatient(patient: Patient) { this.router.navigate(['/admin/patients/edit', patient.user_id]); }
  viewPatient(patient: Patient) { this.router.navigate(['/admin/patients/view', patient.user_id]); }
  viewPatientHistory(patient: Patient) { this.router.navigate(['/admin/patients/history', patient.user_id]); }

  addConsultation() { this.router.navigate(['/admin/consultations/add']); }
  editConsultation(consultation: Consultation) { this.router.navigate(['/admin/consultations/edit', consultation._id]); }
  viewConsultation(consultation: Consultation) { this.router.navigate(['/admin/consultations/view', consultation._id]); }
  viewAllConsultations() { this.router.navigate(['/admin/consultations']); }

...
def create_consultation(current_user):
    ...
    required_fields = ['patient_id', 'doctor_id', 'date', 'motif', 'diagnostic', 'traitement']
...
def get_consultation(current_user, consultation_id):
    try:
        ...
        if current_user['role'] == 'patient' and str(consultation['patient_id']) != str(current_user['_id']):
            return jsonify({'message': 'Non autorisé'}), 403
        
        if current_user['role'] == 'doctor' and str(consultation['doctor_id']) != str(current_user['_id']):
            return jsonify({'message': 'Non autorisé'}), 403
...
def list_consultations(current_user):
    try:
        ...
        consultations = list(mongo.db.consultations.find(filter_query)
                           .sort('date', -1)
                           .skip(skip)
                           .limit(per_page))
        ...
        patients = {str(p['_id']): p for p in mongo.db.patients.find({'_id': {'$in': list(patient_ids)}})}
        doctors = {str(d['_id']): d for d in mongo.db.doctors.find({'_id': {'$in': list(doctor_ids)}})}
